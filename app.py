...
.badge-ok   { display:inline-block; background:#e8f5e9; color:#2e7d32; border:1px solid #a5d6a7; border-radius:20px; padding:0.2rem 0.9rem; font-size:0.85rem; font-weight:600; }
.badge-warn { display:inline-block; background:#fff8e1; color:#e65100; border:1px solid #ffe082; border-radius:20px; padding:0.2rem 0.9rem; font-size:0.85rem; font-weight:600; }
...
if "df_raw" not in st.session_state:
    if st.button("⬇️ Muat Data", type="primary", use_container_width=True):
        bar = st.progress(0)
        try:
            df_raw, nr, nc = load_raw_data(progress_callback=lambda p, t: bar.progress(p, text=t))
            bar.empty()
...
if "tahun" in st.session_state:
    st.markdown(f'<span class="badge-ok">✅ Tahun: {st.session_state["tahun"]}</span>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
<div id="toolbar">
 ...
 <label style="margin-left:6px;">Random: <input id="n-random" type="number" value="100" min="1" style="width:70px;padding:3px 6px;border:1px solid #ccc;border-radius:4px;"></label>
 <button id="btn-random">🎲 Random</button> <button id="btn-clear">🗑 Clear</button> <button id="btn-confirm">✅ Confirm Seleksi</button>
...
function draw(){{
 for(let r=0;r<NR;r++) for(let c=0;c<NC;c++){{
  const idx=r*NC+c; ctx.fillStyle=!available[idx]?C_EMPTY:selected[idx]?C_SELECTED:C_AVAIL;
  ctx.fillRect(c*CELL,r*CELL,CELL-1,CELL-1);
 }}
 document.getElementById('counter').textContent='Terseleksi: '+selected.reduce((s,v)=>s+v,0).toLocaleString();
}}
draw();
const brushSlider=document.getElementById('brush'), brushVal=document.getElementById('brush-val');
brushSlider.addEventListener('input',()=>{{brushVal.textContent=brushSlider.value;}});
function applyBrush(x,y,mode){{
 const col0=Math.floor(x/CELL),row0=Math.floor(y/CELL),R=parseInt(brushSlider.value);
 for(let r=Math.max(0,row0-R);r<=Math.min(NR-1,row0+R);r++) for(let c=Math.max(0,col0-R);c<=Math.min(NC-1,col0+R);c++) if(Math.sqrt((r-row0)**2+(c-col0)**2)<=R){{const idx=r*NC+c;if(available[idx])selected[idx]=mode;}}
}}
let dragging=false,dragMode=0;
canvas.addEventListener('mousedown',e=>{{e.preventDefault();dragging=true;dragMode=e.button===2?0:1;const rect=canvas.getBoundingClientRect();applyBrush(e.clientX-rect.left,e.clientY-rect.top,dragMode);draw();}});
canvas.addEventListener('mousemove',e=>{{if(!dragging)return;const rect=canvas.getBoundingClientRect();applyBrush(e.clientX-rect.left,e.clientY-rect.top,dragMode);draw();}});
window.addEventListener('mouseup',()=>dragging=false); canvas.addEventListener('contextmenu',e=>e.preventDefault());
document.getElementById('btn-random').addEventListener('click',()=>{{selected.fill(0);const a=[];GRID_CELLS.forEach(d=>a.push(d.r*NC+d.c));const n=Math.min(parseInt(document.getElementById('n-random').value)||100,a.length);for(let i=a.length-1;i>0;i--){{const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];}}for(let i=0;i<n;i++)selected[a[i]]=1;draw();}});
document.getElementById('btn-clear').addEventListener('click',()=>{{selected.fill(0);draw();}});
document.getElementById('btn-confirm').addEventListener('click',()=>{{
 const ids=[];for(let r=0;r<NR;r++)for(let c=0;c<NC;c++){const idx=r*NC+c;if(selected[idx]&&cellId[idx])ids.push(cellId[idx]);}
 const msg=document.getElementById('msg');
 if(ids.length===0){{msg.textContent='⚠️ Belum ada sel yang dipilih!';msg.style.color='#e53935';return;}}
 msg.textContent='✅ '+ids.length+' lokasi dikonfirmasi — copy JSON di bawah lalu klik Konfirmasi.'; msg.style.color='#2e7d32';
 document.getElementById('json-out').value=JSON.stringify(ids); document.getElementById('json-wrap').style.display='block';
}});
</script>
<div id="json-wrap" style="display:none;margin-top:8px;"><label style="font-size:12px;color:#555;">📋 Salin JSON ini ke field di bawah canvas:</label><br>
<textarea id="json-out" rows="3" style="width:100%;font-size:11px;font-family:monospace;border:1px solid #a5d6a7;border-radius:6px;padding:6px;background:#f1f8e9;resize:vertical;" readonly></textarea></div>
</body></html>
"""
components.html(CANVAS_HTML, height=700, scrolling=False)

st.markdown("Langkah terakhir: Setelah klik ✅ Confirm Seleksi di canvas, salin JSON yang muncul dan paste ke kolom di bawah ini:")
confirmed_json = st.text_area("JSON ID Lokasi (dari canvas di atas)", value=st.session_state.get("_canvas_json", ""), height=90, placeholder='["LOC_0001","LOC_0023", ...]  ← paste hasil dari canvas', key="canvas_json_input", label_visibility="collapsed")
    ...
    if do_confirm:
        ...
        if not raw:
            ...
            try:
                ...
                st.error("❌ Bukan format JSON yang valid.")

if "sampled_ids" in st.session_state:
    sampled_ids = st.session_state["sampled_ids"]
    st.markdown(f'<span class="badge-ok">✅ {len(sampled_ids):,} lokasi terkonfirmasi</span>', unsafe_allow_html=True)
    tab1, tab2 = st.tabs(["🗺️ Peta Sebaran Sampel", "📈 Preview Time Series"])
    with tab1: st.pyplot(plot_sample_grid(df_year, sampled_ids, nr, nc))
    with tab2: st.pyplot(plot_sample_ts_preview(df_year, sampled_ids, n=3))
    st.markdown('</div>', unsafe_allow_html=True)
else:
    st.markdown('</div>', unsafe_allow_html=True)
...
if "pivot_df" in st.session_state:
    ...
    st.markdown(f'<span class="badge-ok">✅ {n_cls} cluster · {n_noise} noise</span>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
if df_fenologi.empty:
    ...
    st.dataframe(df_fenologi.style.format({'Puncak NDVI': '{:.3f}', 'Min NDVI': '{:.3f}', 'Amplitudo': '{:.3f}', 'Rata-rata NDVI': '{:.3f}', 'Rata-rata StdDev': '{:.3f}', 'Jml Titik': '{:,.0f}'}).background_gradient(cmap='YlGn', subset=['Puncak NDVI', 'Amplitudo']), use_container_width=True)
    st.subheader("📊 Grafik Time Series")
    col1, col2 = st.columns(2)
    with col1: st.pyplot(plot_comparison(cluster_ts, valid_statuses, tahun))
    with col2: st.pyplot(plot_individual_clusters(cluster_ts, valid_statuses))
