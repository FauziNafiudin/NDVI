...
from clustering import compute_dtw, run_hdbscan, get_dtw_description
from visualization import (
    plot_grid_preview, plot_smoothing_preview,
    plot_sample_grid, plot_sample_ts_preview,
    calculate_metrics, plot_comparison, plot_individual_clusters, plot_spatial_map,
)
...
.step-card {
    ...
    border-radius: 10px; padding: 1.4rem 1.8rem 1rem; margin-bottom: 0.5rem; background: #f9fff9;
}
...
# Handle auto-confirmation from Canvas via URL params
q = st.query_params
if "sampled" in q:
    try:
        ids = json.loads(q["sampled"])
        st.session_state["sampled_ids"] = ids
        q.clear()
        st.rerun()
    except Exception as e:
        st.error(f"Error parsing query param: {e}")
...
if "df_raw" not in st.session_state:
    ...
    st.markdown("🗺️ **Peta Sebaran Seluruh Lokasi (Sebelum Filter Tahun)**")
...
for yr, col in [("2023", c1), ("2024", c2)]:
    ...
    if col.button(f"📆 {yr}", type="primary" if is_active else "secondary", use_container_width=True):
        if st.session_state.get("tahun") != yr:
            for k in ["df_smooth", "df_year", "sampled_ids", "pivot_df", "dtw_matrix"]:
...
if st.button("▶ Jalankan Smoothing", type="primary", use_container_width=True):
    ...
    for k in ["sampled_ids", "pivot_df", "dtw_matrix"]:
        st.session_state.pop(k, None)
    st.rerun()
...
if "df_smooth" in st.session_state:
    ...
    st.markdown("📈 **Preview Hasil Smoothing vs Data Asli**")
    st.pyplot(plot_smoothing_preview(df_year, n=2))
    st.markdown('</div>', unsafe_allow_html=True)
else:
    st.markdown('</div>', unsafe_allow_html=True)
...
st.caption("Klik-drag untuk memilih lokasi. Klik kanan untuk deselect. Klik ✅ Confirm untuk langsung lanjut.")
...
body {{ font-family: sans-serif; background: #f9fff9; padding: 4px; overflow: hidden; }}
#toolbar {{
...
#counter {{
    font-weight: 700; color: #2e7d32; font-size: 13px;
...
#canvas-wrap {{
    ...
    background: #fff; width: 100vw; height: calc(100vh - 60px);
}}
canvas {{ display: block; cursor: crosshair; }}
#msg {{ margin-top: 4px; font-size: 11px; color: #555; text-align: center; }}
</style>
</head>
<body>
<div id="toolbar">
 <label>Brush: <input id="brush" type="range" min="1" max="10" value="3" style="width:80px;vertical-align:middle;"></label>
 <label style="margin-left:6px;">Random: <input id="n-random" type="number" value="50" min="1" style="width:60px;padding:2px;border:1px solid #ccc;border-radius:4px;"></label>
...
<div id="msg">💡 Klik kiri drag = pilih | Klik kanan drag = hapus</div>
<script>
const NR = {nr_js}; const NC = {nc_js};
const GRID_CELLS = {grid_json}; const INIT_SELECTED = {init_selected};
const C_EMPTY = '#D3D3D3'; const C_AVAIL = '#C8E6C9'; const C_SELECTED = '#2196F3';
const available = new Uint8Array(NR * NC); const selected = new Uint8Array(NR * NC);
const cellId = new Array(NR * NC).fill(null);
GRID_CELLS.forEach(d => {{ const idx = d.r * NC + d.c; available[idx] = 1; cellId[idx] = d.id; }});
new Set(INIT_SELECTED).forEach(id => {{
    const idx = GRID_CELLS.find(c=>c.id===id);
    if(idx) selected[idx.r*NC+idx.c]=1;
}});
const canvas = document.getElementById('c'); const ctx = canvas.getContext('2d');
const CELL = Math.max(4, Math.floor((window.innerWidth - 20) / NC));
canvas.width = NC * CELL; canvas.height = NR * CELL;
function draw() {{
    for(let r=0;r<NR;r++) for(let c=0;c<NC;c++) {{
        const idx=r*NC+c;
        ctx.fillStyle=!available[idx]?C_EMPTY:selected[idx]?C_SELECTED:C_AVAIL;
        ctx.fillRect(c*CELL, r*CELL, CELL-1, CELL-1);
    }}
    document.getElementById('counter').textContent='Terseleksi: '+selected.reduce((a,b)=>a+b,0).toLocaleString();
}}
draw();
function getBrushR(){{ return parseInt(document.getElementById('brush').value); }}
function applyBrush(x,y,mode){{
    const col0=Math.floor(x/CELL); const row0=Math.floor(y/CELL); const R=getBrushR();
    for(let r=Math.max(0,row0-R);r<=Math.min(NR-1,row0+R);r++) {{
        for(let c=Math.max(0,col0-R);c<=Math.min(NC-1,col0+R);c++) {{
            if(Math.sqrt((r-row0)**2+(c-col0)**2)<=R && available[r*NC+c]) selected[r*NC+c]=mode;
        }}
    }}
}}
let dragging=false, dragMode=0;
canvas.addEventListener('mousedown', e=>{{ e.preventDefault(); dragging=true; dragMode=e.button===2?0:1;
    const rect=canvas.getBoundingClientRect(); applyBrush(e.clientX-rect.left, e.clientY-rect.top, dragMode); draw(); }});
canvas.addEventListener('mousemove', e=>{{ if(!dragging)return;
    const rect=canvas.getBoundingClientRect(); applyBrush(e.clientX-rect.left, e.clientY-rect.top, dragMode); draw(); }});
window.addEventListener('mouseup',()=>{{dragging=false;}});
canvas.addEventListener('contextmenu',e=>e.preventDefault());
document.getElementById('btn-random').addEventListener('click',()=>{{
    selected.fill(0); const avail=[];
    GRID_CELLS.forEach(d=>avail.push(d.r*NC+d.c));
    const n=Math.min(parseInt(document.getElementById('n-random').value)||50, avail.length);
    for(let i=avail.length-1;i>0;i--){{const j=Math.floor(Math.random()*(i+1));[avail[i],avail[j]]=[avail[j],avail[i]];}}
    for(let i=0;i<n;i++) selected[avail[i]]=1; draw();
}});
document.getElementById('btn-clear').addEventListener('click',()=>{{ selected.fill(0); draw(); }});
document.getElementById('btn-confirm').addEventListener('click',()=>{{
    const ids=[];
    for(let r=0;r<NR;r++) for(let c=0;c<NC;c++) {{ const idx=r*NC+c; if(selected[idx]&&cellId[idx]) ids.push(cellId[idx]); }}
    if(ids.length===0){{ document.getElementById('msg').textContent='⚠️ Belum ada yang dipilih!'; return; }}
    document.getElementById('msg').textContent='✅ ' + ids.length + ' lokasi dikonfirmasi — Memproses...';
    const url = new URL(window.parent.location.href);
    url.searchParams.set('sampled', JSON.stringify(ids));
...
if "sampled_ids" in st.session_state:
    ...
    st.markdown(f'<span class="badge-ok">✅ {len(sampled_ids):,} lokasi terkonfirmasi otomatis</span>',
                unsafe_allow_html=True)
    ...
    st.info("👆 Gambar lokasi di canvas di atas, lalu klik ✅ Confirm.")
...
# ═══════════════════════════════════════════════════════
st.markdown('<hr class="step-divider">', unsafe_allow_html=True)
st.markdown('<div class="step-card"><h3>📊 Step 5 — Hitung Matriks DTW</h3>', unsafe_allow_html=True)
sampled_ids = st.session_state["sampled_ids"]
...
if st.button("🔢 Jalankan Hitung DTW", type="primary", use_container_width=True):
    bar = st.progress(0)
    dist_matrix, locs_order = compute_dtw(
        df_sampled, progress_callback=lambda p, t: bar.progress(p, text=t))
    bar.empty()
    st.session_state["dtw_matrix"] = dist_matrix
    st.session_state["locs_order"] = locs_order.tolist()
    st.rerun()

if "dtw_matrix" in st.session_state:
    st.markdown('<span class="badge-ok">✅ Matriks DTW siap untuk clustering</span>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
if "dtw_matrix" not in st.session_state:
    st.info("👆 Klik tombol di atas untuk menghitung DTW.")
    st.stop()
...
# ═══════════════════════════════════════════════════════
st.markdown('<hr class="step-divider">', unsafe_allow_html=True)
st.markdown('<div class="step-card"><h3>🔍 Step 6 — Parameter & Jalankan HDBSCAN</h3>', unsafe_allow_html=True)
...
if st.button("🚀 Jalankan Clustering HDBSCAN", type="primary", use_container_width=True):
    bar = st.progress(0)
    labels = run_hdbscan(
        st.session_state["dtw_matrix"], min_cluster_size, min_samples, epsilon,
        progress_callback=lambda p, t: bar.progress(p, text=t))
    bar.empty()
    
    # Buat pivot_df dengan hasil clustering
    pivot_df = pd.DataFrame(index=st.session_state["locs_order"])
...
if "pivot_df" not in st.session_state:
    st.info("👆 Atur parameter dan klik Jalankan.")
    st.stop()
...
if df_fenologi.empty:
    ...
    # df_year digunakan untuk mengetahui semua lokasi yang tersedia agar bisa diwarnai hijau pudar
    st.pyplot(plot_spatial_map(df_labeled, df_year, nr, nc, valid_statuses))
