...
def compute_dtw(df_sampled, progress_callback=None):
    ...
    if progress_callback: progress_callback(1.0, "✅ DTW selesai!")
    return dist_matrix, pivot_df.index

def run_hdbscan(dist_matrix, min_cluster_size=3, min_samples=2,
                cluster_selection_epsilon=0.05, progress_callback=None):
    if progress_callback: progress_callback(0.1, "🔍 Menjalankan HDBSCAN...")
